...
freq1 = zeros(num_of_tests, 1) ;
freq2 = zeros(num_of_tests, 1) ;
...
for jj=1:length(SNR_dB)
    ...
    for k=1:num_of_tests 
        ...
        freq1(k) = omega0_1*fd/2/pi ;

        if (abs(freq1(k)-fs) <= pll_line)
        ...
        freq2(k) = omega0_2*fd/2/pi ;

        if (abs(freq2(k)-fs) <= pll_line)
    ...
    freq1_match(jj) = freq1_match(jj) / num_of_tests * 100 ;
    freq2_match(jj) = freq2_match(jj) / num_of_tests * 100 ;
    freq3_match(jj) = freq3_match(jj) / num_of_tests * 100 ;
