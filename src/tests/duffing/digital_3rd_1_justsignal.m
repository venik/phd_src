...
% 3rd order
% [1  x_k  x_k-1  x_k^2  x_k*x_k-1  x_k-1^2] 2nd order
% [x_k^3  x_k^2*x_k-1  x_k*x_k-1^2  x_k-1^3] 3rd order
length_of_vec = 10 ;
...
for k = 1:num_eq
    ...
    % adjust coef for 3rd
    mtx_coef(k, 7) = mtx_coef(k, 2)^3 ;
    mtx_coef(k, 8) = mtx_coef(k, 3) * mtx_coef(k, 2)^2 ;
    mtx_coef(k, 9) = mtx_coef(k, 2)^2 * mtx_coef(k, 2) ;
    mtx_coef(k, 10) = mtx_coef(k, 3)^3 ;
end ;  % for k
...
for k = 2:length(t)
    y_volterra(k) = h(1) + h(2) * x_v(k) + h(3) * x_v(k-1) + h(4) * x_v(k)^2 + h(5) * x_v(k) * x_v(k-1) + h(6) * x_v(k-1)^2 + h(7) * x_v(k)^3 + h(8) * x_v(k-1) * x_v(k)^2 + h(9) * x_v(k-1)^2 * x_v(k) + h(10) * x_v(k-1)^3;
end % for
...
% plot
    clf; figure(1),
        ...
        title('Only signal, order 3, length 1');
