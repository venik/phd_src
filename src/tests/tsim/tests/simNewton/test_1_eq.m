...
if numel(n2) == 1
    ...
    xlim([0 8000]), xlabel('Частота, Гц', 'Fontsize', 14), ylabel('A', 'Fontsize', 14) ; 
    title(sprintf('Итерации поиска решения нелинейного уравнения 14, смещения %d и %d', tau1, tau2), ...
            'Fontsize', 14);
    legend('Функция', 'Частота сигнала', ...
            'Оценка частоты аналитическим методом', ...
            'Оценка частоты аналитическим методом', ...
            'Оценка частоты методом ньютона')
    grid on ;
...
if A>0
    figure(1), grid on,
        subplot(2,1,1),
            ...
            legend('Ньютон', 'Аналитическое решение');
        subplot(2,1,2),
